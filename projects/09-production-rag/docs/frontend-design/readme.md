...
09 的前端不是教学 Demo 页面，而是一个面向真实用户的知识库问答系统：
...
- 顶部按钮：`+ 添加来源`
- 搜索来源区域：保留视觉，但 Web / Fast Research 禁用
- 空状态图标
- 文案：`已保存的来源将显示在此处`
...
- `上传文件` 可用。
- `粘贴文本` 可选，如果实现成本低，可打开一个 textarea 并生成临时 txt 文档。
- `网站`、`云端硬盘` 禁用，显示 `即将支持`。
- 拖拽文件进入区域时高亮边框。
- 上传后显示文件队列和解析状态。
...
- 用 mock 数据复现 `1.png`、`3.png`、`4.png`、`5.png`、`7.png`、`8.png`。
