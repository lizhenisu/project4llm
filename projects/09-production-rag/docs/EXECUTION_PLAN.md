...
> 4. **全局多轮循环升华 (Iterative Refinement)**：当跑完整个计划的全部阶段（完成当前轮次）后，**必须反过头来，从阶段一重新开始执行第二轮（Round 2）、第三轮（Round 3）……**。在后续轮次中，由 AI 自主审视粗糙代码，集中精力重构优化、打磨 UI 细节、强化高并发健壮性和安全性。如此循环多轮，**直到 AI 评估当前代码架构、容错率和体验已经达到生产级完整度，方可宣告整个史诗级更新结束**。
