...
> 1. **大步快跑，按“阶段”整体交付**：首轮开发（Round 1）以构建全栈贯通的 MVP（最小可行性产品）为首要目标。按阶段提交，严禁在某一阶段死磕底层细节。
> 2. **先跑通，再跑好 (Make it work, then make it right)**：在首轮推进时，只要核心逻辑无报错跑通即可。对于深度的异常捕获、极致的性能抽象、或 UI 的“锦上添花”，**强制推迟到下一轮次进行**，绝不允许其阻碍当前轮次的全局推进。
> 3. **阶段性 Checkpoint**：每个阶段完成后，必须保证系统全栈可用，并执行 Git Commit 作为回滚基线。
> 4. **全局多轮循环升华 (Iterative Refinement)**：当跑完整个计划的全部阶段（完成当前轮次）后，**必须反过头来，从阶段一重新开始执行第二轮（Round 2）、第三轮（Round 3）……**。在后续轮次中，由 AI 自主审视粗糙代码，集中精力重构优化、打磨 UI 细节、强化高并发健壮性和安全性。如此循环多轮，**直到 AI 评估当前代码架构、容错率和体验已经达到无懈可击的生产级完整度，方可宣告整个史诗级更新结束**。
