...
async def run_load(args: argparse.Namespace, questions: list[str]) -> dict[str, Any]:
    if args.warmup > 0:
        await run_batch(args, questions, total=args.warmup, label="warmup")

    started = time.perf_counter()
    samples = await run_batch(args, questions, total=args.requests, label="load")
    wall_ms = elapsed_ms(started)
    return build_summary(args, samples, wall_ms=wall_ms)
    ...
    *,
    ...
    async def run_one(index: int) -> RequestSample:
        async with semaphore:
            ...
            sample = await asyncio.to_thread(send_stream_request, args, question, index, label)
