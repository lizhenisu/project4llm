...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        os.environ["MILVUS_URI"] = str(Path(tmp) / "multimodal_answer.db")
        os.environ["RAG_COLLECTION"] = "rag_smoke_multimodal_answer"
...
def run_smoke() -> None:
    ...
    result = answer_multimodal_query(
        ...
        candidate_limit=5,
        context_limit=3,
        request_id="smoke-multimodal-answer",
    )
    assert result.hits
    assert result.hits[0].doc_id == "dashboard-screenshot"
    assert result.trace.retrieval_mode == "multimodal_text_image_fusion_rerank"
