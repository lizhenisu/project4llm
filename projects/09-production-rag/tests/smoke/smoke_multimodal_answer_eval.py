...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        os.environ["MILVUS_URI"] = str(Path(tmp) / "multimodal_answer_eval.db")
        os.environ["RAG_COLLECTION"] = "rag_smoke_multimodal_answer_eval"
...
def run_smoke(tmp_dir: Path) -> None:
    ...
    eval_path = tmp_dir / "multimodal_answer_eval.jsonl"
    write_jsonl(
        eval_path,
        [
            {
                ...
                "answerable": True,
                "expected_answer_terms": ["RAG Dashboard", "latency", "recall"],
    ...
    print("smoke_multimodal_answer_eval=ok")
