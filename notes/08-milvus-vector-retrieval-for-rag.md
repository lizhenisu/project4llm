...
1. demo 的 collection 是 `rag_chunks_demo`，向量维度是 64，metric 是 `COSINE`。每条记录对应一个知识库 chunk，包含 `id`、`vector`、`text`、`doc_id`、`tenant_id`、`source` 等字段。`id` 用于唯一定位，`vector` 用于语义检索，`text` 用于组 prompt，`doc_id` 用于溯源和更新，`tenant_id` / `source` 用于权限和来源过滤。面试中要强调：RAG schema 不是只存向量，还要存足够的 metadata 支撑过滤、审计、增量更新和排障。
2. `COSINE` 比较的是向量方向，适合文本 embedding。demo 的 `hash_embedding` 会把向量除以 L2 norm 做归一化，这样长度差异不会主导相似度。真实项目中要让 embedding 模型、向量归一化策略和 Milvus metric 保持一致；如果用 `IP` 模拟 cosine，也需要先归一化。
3. 不加 filter 时，Milvus 会在整个 collection 内检索，可能返回其他租户、其他来源或当前业务不可见的 chunk。加 `tenant_id` filter 后，候选集先被限制在指定租户内，再做向量相似度排序，结果更符合权限边界。生产系统中权限过滤应放在检索阶段，而不是检索后在应用层丢弃，否则 topK 可能先被无权限数据占满。
4. topK 增大时，正确 chunk 进入候选集的概率通常提高，所以 `recall@k` 可能上升。但 topK 过大会增加检索延迟、rerank 成本和上下文噪声。工程上通常扫描不同 topK，观察 `recall@k`、MRR、p95 latency 和最终回答质量，再选一个平衡点。
5. hash embedding 的优点是无需下载模型、运行稳定、适合演示 collection、insert、search、filter 和 recall 计算链路；缺点是语义表达能力很弱，不能代表真实 RAG 质量。真实项目应使用经过训练的 embedding 模型，例如 `bge`、`e5` 或商业 embedding API，并用业务评估集验证召回和端到端回答效果。
